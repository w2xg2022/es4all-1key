...
if [ -n "$SELF_DIR" ] && [ -f "$SELF_DIR/scripts/00-common.sh" ]; then
    ...
    echo "[1key] 下载安装脚本..."
    for f in 00-common.sh platforms.sh 01-prep.sh 02-bootsplash.sh 03-retroarch.sh 04-emulationstation.sh 05-autostart.sh 06-profiles.sh; do
...
# ★放在最后★：它要把机型专属配置落到 game 家目录，前面的阶段得先把使用者与
# ~/.emulationstation 建好；apply.sh 也会读 ES 的设定档。
# 注：原本的阶段 6（手柄键位同步）已移除，键位透传现在整条来自 es4all-profiles，
#     由本阶段下发。见 06-profiles.sh 档首。
bash "$SCRIPT_DIR/06-profiles.sh"
