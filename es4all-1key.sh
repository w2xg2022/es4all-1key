...
if [ -n "$SELF_DIR" ] && [ -f "$SELF_DIR/scripts/00-common.sh" ]; then
    ...
    echo "[1key] 下载安装脚本..."
    for f in 00-common.sh platforms.sh 01-prep.sh 02-bootsplash.sh 03-retroarch.sh 04-emulationstation.sh 05-autostart.sh 07-profiles.sh; do
...
#     由本阶段下发。见 07-profiles.sh 档首。
bash "$SCRIPT_DIR/07-profiles.sh"
